...
export function TaskView({ taskId }: TasKViewProps) {
  ...
  const functionData = useMemo(() => {
    ...
    const decodedArgs = decodeModuleArgs(moduleArgs as string[], modules as number[])

    if (taskResolver && resolverAbi && taskResolver.address) {
      return {
        args: decodedArgs,
        data: getTaskFunctionData(taskResolver.address, resolverAbi, decodedArgs.resolverData as string),
      }
    }
    ...
    return {
      args: decodedArgs,
      data: getTaskFunctionData(taskWithName.task.execAddress, abi, taskWithName.task.execDataOrSelector as string),
    }
          ...
          <div className="flex items-center space-x-5">
            ...
            <div>
              {showRename ? (
                ...
                <FaEdit className="cursor-pointer opacity-50 duration-200 hover:opacity-100" size={24} onClick={() => setShowRename(true)} />
              )}
            </div>
            <div>
        ...
        <div className="card mx-auto mt-10 w-full !max-w-4xl !rounded-xl !border-none !px-10 py-8 dark:!bg-zinc-900">
          <div className="flex flex-col space-y-5">
            ...
            {!taskResolver?.address ? (
              <>
                {functionData && <FunctionData functionData={functionData.data} />}
                <hr className="!mt-10 !mb-5" />
                {functionData && <InputValues functionData={functionData.data} />}
                <hr className="!mt-10 !mb-5" />
                <IntervalValues createdAt={task.createdAt} interval={functionData?.args.interval} startTime={functionData?.args.startTime} />
              </>
            ) : (
              <>
                <hr className="!mt-10 !mb-5" />
                {functionData && <ResolverValues functionData={functionData.data} resolverAddress={taskResolver.address} />}
              </>
            )}
            <hr className="!mt-10 !mb-5" />
