...
import {
  ...
  getTotalInterval,
  inputsAreFilled,
} from "@/integrations/gelato/utils/helpers"
...
export const useWizard = (form: UseFormReturn<CreateTaskForm>) => {
  ...
  const allPredefinedInputsAreFilled = useMemo(() => {
    ...
    try {
      return inputsAreFilled(
        getAbiFunctions(JSON.parse(abi) as Abi).find(
          (item) => item.name === func
        ) as AbiFunction,
  ...
  const allResolverInputsAreFilled = useMemo(() => {
    ...
    try {
      return inputsAreFilled(
        getAbiFunctions(JSON.parse(resolverAbi) as Abi).find(
          (item) => item.name === resolverFunc
        ) as AbiFunction,
  ...
  const totalInterval = useMemo(
    () => getTotalInterval(d, h, m, s),
    [d, h, m, s]
  )

  const shouldShowFunction = !!abi && !!contractAddress
  const shouldShowInputs = shouldShowFunction && !!func
  const shouldShowResolverInputs =
    shouldShowInputs && inputDefinition === "resolver"
  const allInputsAreFilled =
    inputDefinition === "resolver"
      ? allResolverInputsAreFilled
      : allPredefinedInputsAreFilled

  const shouldShowRestrictionInfo = shouldShowInputs && !!allInputsAreFilled
  const shouldShowIntervalInput =
    shouldShowRestrictionInfo && inputDefinition === "predefined"

  const intervalIsValid = shouldShowIntervalInput ? totalInterval >= 25 : true

  const shouldShowPayment =
    shouldShowRestrictionInfo &&
    (intervalIsValid || timeOption == "whenever_possible")
