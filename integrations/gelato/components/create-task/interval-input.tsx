...
export function IntervalInput() {
  ...
  const [timeOption, days, hours, minutes, seconds, startImmediately, startTime] = watch([
    'timeOption',
    'timeInterval.days',
    'timeInterval.hours',
  ...
  const totalInterval = useMemo(() => getTotalInterval(days, hours, minutes, seconds), [days, hours, minutes, seconds])
  ...
  const firstExecution = useMemo(() => moment(date).format('ll, HH:mm:ss'), [date])
    ...
    <div className="card mx-auto mt-10 w-full !max-w-4xl !rounded-xl !border-none !px-10 py-8 dark:!bg-zinc-900">
      ...
      <div className="mt-10">
        {timeOption === 'exact' ? (
            ...
            <div className="mt-5">
              ...
              <div className="mt-2 flex items-center space-x-5">
                <input
                  ...
                  placeholder="0 days"
                  {...register('timeInterval.days')}></input>
                <span>+</span>
                <input
                  className="input !rounded-2xl dark:!bg-zinc-700 dark:!text-white"
                  placeholder="0 hours"
                  {...register('timeInterval.hours')}></input>
                <span>+</span>
                <input
                  className="input !rounded-2xl dark:!bg-zinc-700 dark:!text-white"
                  placeholder="0 min"
                  {...register('timeInterval.minutes')}></input>
                <span>+</span>
                <input
                  className="input !rounded-2xl dark:!bg-zinc-700 dark:!text-white"
                  placeholder="0 sec"
                  {...register('timeInterval.seconds')}></input>
              </div>
            </div>
            ...
            {totalInterval > 0 && (
              <div className="mt-5 flex flex-col space-y-3">
                <div className="flex space-x-3">
                  ...
                  <span>{startImmediately ? firstExecution : moment(startTime).format('ll, HH:mm:ss')}</span>
                </div>
                <div className="flex space-x-3">
                  <span>2nd execution:</span>
                  <span>{startImmediately ? secondExecution : moment(startTime).add(totalInterval, 'seconds').format('ll, HH:mm:ss')}</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div>
            <div>Gelato will check if the function is executable at every block.</div>
            <div className="mt-3 dark:opacity-50">
              Checking the function will start immediately after the task is created. Make sure the function can only be executed from time to time
              and not always.
            </div>
          </div>
        )}
