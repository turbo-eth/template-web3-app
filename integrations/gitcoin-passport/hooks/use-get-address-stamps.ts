...
export const useGetAddressStamps = () => {
  ...
  const { isLoading, data: stamps, error, refetch: refetchQuery, isRefetching } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (response.status === 200) {
        ...
        data.items.forEach(userStamp => {
          ...
          const providerObject = userStamps.find(userStamp => userStamp.provider === provider)
          const stampId = userStamp.credential.credentialSubject.provider
          if (providerObject) providerObject.items = [...providerObject.items, stampId]
