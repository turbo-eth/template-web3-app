...
export function Erc1155WriteApprove({ address }: Erc1155WriteApproveProps) {
  ...
  const { config, error, isError } = usePrepareErc1155Approve({
    address,
    args: debouncedToAddress && debouncedTokenId ? [debouncedToAddress, BigInt(debouncedTokenId), BigInt(debouncedTargetValue)] : undefined,
    enabled: Boolean(debouncedToAddress && debouncedTokenId && debouncedTargetValue),
  })
    ...
    <div className="card w-full">
      <form className="flex flex-col gap-4" onSubmit={handleSubmit(onSubmit)}>
        ...
        <TransactionStatus
          ...
          hash={data?.hash}
          isError={isError && Boolean(debouncedToAddress && debouncedTokenId && debouncedTargetValue)}
