...
import { useErc1155SafeTransferFrom, usePrepareErc1155SafeTransferFrom } from '../generated/erc1155-wagmi'
...
export function Erc1155WriteTransfer({ address }: Erc1155WriteTransferProps) {
  ...
  const { data, write, isLoading: isLoadingWrite } = useErc1155SafeTransferFrom(config)
    ...
    <div className="card w-full">
      <form className="flex flex-col gap-4" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex items-center justify-between">
          ...
          <p className="text-center text-sm text-gray-500">Transfer NFTs or FTs to any address</p>
        </div>
      </form>
    </div>
  )
