...
import { useErc1155SetApprovalForAll, usePrepareErc1155SetApprovalForAll } from '../generated/erc1155-wagmi'

interface Erc1155WriteSetApprovalForAllProps {
  address: Address
}

export function Erc1155WriteApproveForAll({ address }: Erc1155WriteSetApprovalForAllProps) {
  ...
  const { config, error, isError } = usePrepareErc1155SetApprovalForAll({
    address,
    args: debouncedToAddress && debouncedShouldApproved ? [debouncedToAddress, debouncedShouldApproved] : undefined,
    enabled: Boolean(debouncedToAddress && debouncedShouldApproved),
  })

  const { data, write, isLoading: isLoadingWrite } = useErc1155SetApprovalForAll(config)
  ...
  return (
    <div className="card w-full">
      <form className="flex flex-col gap-4" onSubmit={handleSubmit(onSubmit)}>
        ...
        <label>Approve?</label>
        <input type="checkbox" {...register('shouldApproved')} className="input" />
        ...
        <TransactionStatus
          ...
          hash={data?.hash}
          isError={isError && Boolean(debouncedToAddress && debouncedShouldApproved)}
        ...
        <div className="flex items-center justify-between">
          ...
          <p className="text-center text-sm text-gray-500">Approve all tokens to any address</p>
        </div>
      </form>
    </div>
  )
