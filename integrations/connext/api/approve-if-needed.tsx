...
export async function POST(req: Request) {
  try {
    ...
    if (!originDomain) {
      return new Response("Missing origin domain in request body", {
        status: 400,
      })
    }

    if (!assetAddress) {
      return new Response("Missing assetAddress in request body", {
        status: 400,
      })
    }
    ...
    const tx =
      environment === "mainnet"
        ? await mainnetSdk.sdkBase.approveIfNeeded(
            originDomain,
            assetAddress,
            amount
          )
        : await testnetSdk.sdkBase.approveIfNeeded(
            originDomain,
            assetAddress,
            amount
          )
