...
export async function GET(req: Request) {
  try {
    ...
    if (!originTokenAddress) {
      return new Response("Missing origin token address query parameter", {
        status: 400,
      })
    }
    ...
    const { amountReceived, isFastPath } =
      environment === "mainnet"
        ? await mainnetSdk.sdkBase.calculateAmountReceived(
            originDomain,
            destinationDomain,
            originTokenAddress,
            amount,
        ...
        : await testnetSdk.sdkBase.calculateAmountReceived(
            originDomain,
            destinationDomain,
            originTokenAddress,
            amount,
    ...
    return new Response(
      JSON.stringify({ amount: amountReceived.toString(), isFastPath }),
      {
        status: 200,
